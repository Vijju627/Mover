...
@bot.command(name="throughput", aliases=["testspeed"])
async def throughput(ctx, count: int = 10000):
    """Measure local async-loop throughput without sending Discord messages."""
    if not is_owner(ctx):
        return await ctx.send(embed=error_embed("You do not have permission to use this command."))
    ...
    await ctx.send(embed=success_embed(
        f"⚡ Local throughput test: **{processed:,}** operations in "
        f"**{elapsed:.3f}s** (~**{rate:,.0f} ops/s**). No Discord messages were sent."
    ))
