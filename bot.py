...
async def on_message(message):
    ...
    # Prefixless commands are available to the configured owner only.
    # Normalize command names so lowercase, UPPERCASE, and MixedCase work.
    if message.author.id in BOT_OWNER_IDS:
        parts = content.split()
        if parts:
            command = parts[0].lower()

            if command in {"vc", "voice"}:
                if len(parts) >= 2:
                    parts[1] = parts[1].lower()
                message.content = PREFIX + " ".join(parts)

            elif command in {
                "link", "spam", "stopspam",
                "throughput", "testspeed"
            }:
                parts[0] = command
                message.content = PREFIX + " ".join(parts)
